...
\begin{frame}{Xenon Stripping Dynamics}
    \begin{columns}
        \begin{column}{0.5\textwidth}
            \begin{figure}[ht!]
                \centering
                \includegraphics[width=\textwidth]{Xenon/8hr_restart}{Concentration of \I and \Xe vs. time following reactor scram - Restart Mode}
            \end{figure}
        \end{column}
        \begin{column}{0.5\textwidth}
            \begin{figure}[ht!]
...
\begin{frame}{Passive Feedback\only<2-3>{\ST}}{\only<1>{\cST}\only<2>{Temperature Reactivity}\only<3>{Flow Reactivity}}
    \only<1>{
     \begin{figure}[!ht]
         \centering
         \input{./tikz/passive}
